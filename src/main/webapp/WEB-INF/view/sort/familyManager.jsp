...
<div style="display: none;" id="newsToolBar">
    <c:if test="${role==1}">
        <button type="button" class="layui-btn layui-btn-sm layui-btn-radius" lay-event="add">新增科目</button>
    </c:if>
</div>
    ...
    <form class="layui-form" lay-filter="dataFrm" id="dataFrm" style="margin-right: 20px">
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="familyId" placeholder="请输入科目编号" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">科目名称:</label>
...
<div style="display: none;padding: 20px" id="saveOrUpdateDiv2">
    <form class="layui-form" lay-filter="dataFrm2" id="dataFrm2" style="margin-right: 20px">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text"  name="genusId" placeholder="请输入属目编号" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">属目名称:</label>
            <div class="layui-input-block">
                <input type="text" id="genusName" name="genusName" placeholder="请输入属目名称" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block" style="text-align: center;padding-right: 120px">
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate', 'upload'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '/sort/familyList.action' //数据接口
            , title: '科目分类信息'//数据导出来的标题
            , toolbar: "#newsToolBar"   //表格的工具条
            , height: 'full-220'
            , cellMinWidth: 120 //设置列的最小默认宽度
            , page: true  //是否启用分页
            , cols: [[   //列表数据
                {field: 'familyId', title: '科目编号', align: 'center'}
        ...
        $("#doSearch").click(function () {
            ...
            //alert(params);
            tableIns.reload({
                url: "/sort/familyList.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(newsTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.familyName+ '】这个植物以及对应的信息么？', function (index) {
                    //向服务端发送删除指令
                    $.post("/sort/deleteFamily.action", {familyId: data.familyId}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
